...
if (Input::get('do') == 'DynamicPages')
{
	System::loadLanguageFile('tl_dynamicregion_item');

	array_unshift($GLOBALS['TL_DCA']['tl_content']['config']['onload_callback'], array('tl_content_DynamicPages', 'checkPermission'));
}
 ...
 * @internal
 */
class tl_content_DynamicPages extends Backend
{
	/**
	...
	protected function checkAccessToElement($id, $root, $blnIsPid=false)
	{
		if ($blnIsPid)
		{
			$objArchive = Database::getInstance()
				->prepare("SELECT a.id, n.id AS nid FROM tl_dynamicregion_item n, tl_dynamicregion_archive a WHERE n.id=? AND n.pid=a.id")
				->limit(1)
				->execute($id);
		}
		else
		{
			$objArchive = Database::getInstance()
				->prepare("SELECT a.id, n.id AS nid FROM tl_content c, tl_dynamicregion_item n, tl_dynamicregion_archive a WHERE c.id=? AND c.pid=n.id AND n.pid=a.id")
				->limit(1)
				->execute($id);
		}
